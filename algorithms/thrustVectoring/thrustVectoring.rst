...
.. list-table:: Module I/O Messages
    ...
    * - thrusterConfigFInMsg
      ...
      - Input thruster configuration message, in **platform frame coordinates**. The module takes only
        ``maxThrust`` from it. The entry ``rThrust_B`` here is the position of the thrust application point with
        respect to the origin of the platform frame, in platform-frame coordinates
        (:math:`{}^\mathcal{F}\boldsymbol{r}_{T/F}`). **Read one time at** ``reset()``.
    * - cmdTorqueInMsg
...
.. list-table:: Module Parameters
    ...
    * - ``armLength``
      - 0
      - :math:`\geq 0`
      - distance [m] from :math:`M` to the thrust application point, along the thrust. It changes only the
        application point in the output message, never the force or the torque
...
The two remaining fields of ``thrusterConfigFInMsg`` are a **mounting contract**, not free parameters. The module
examines them and does not use them. ``rThrust_B`` must be zero, and ``tHatThrust_B`` must be :math:`[0, 0, 1]`,
because all the mathematics below holds only for a thrust line of action that goes through :math:`M`. The module
rejects a different thruster description, such as an offset nozzle or a canted one, at ``reset()``. It does not
give an incorrect direction for a thruster that the spacecraft does not have.
...
^^^^^^^^^^^^^^^^^^^^^^^^^^^
The thruster is attached to a platform that turns about the hub-fixed point :math:`M`. The thruster sits on the
platform frame :math:`\mathcal{F}` axis :math:`+z_\mathcal{F}` and fires along it, at the distance ``armLength``
from :math:`M`. The line of action therefore goes through :math:`M` for every platform orientation.
...
:math:`\boldsymbol{r}_{M/C}` is hub-fixed. The platform therefore changes the torque **only** through the thrust
...
This leaves the module with no platform frame at run time. The center of mass, the point :math:`M` and the
requested torque are all body-frame quantities, and the module calculates the direction directly in body-frame
coordinates. It turns nothing each cycle.
...
``bodyHeadingOutMsg`` and to ``thrusterConfigBOutMsg``, with no rotation, and gives the magnitude as configured.
...
with :math:`\ell` the arm length. The module writes it to ``thrusterConfigBOutMsg`` as ``rThrust_B``. This is the
...
that is when the module reads them::
    ...
    thrustVectoring.thrusterConfigFInMsg.subscribeTo(thrConfigFMsg)
...
----------------------------------
**Assumption.** The thrust line of action goes through the point :math:`M`. The thruster fires along
...
thruster the direction is incorrect. ``reset()`` can compare only the thruster description in the input message
with this assumption (see *Module Parameters*).
...
torque, and not the length of the platform arm.
...
thrust far from the neutral direction of the platform. A downstream module must hold the deflection inside the
